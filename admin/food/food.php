...
// Pagination
$results_per_page = 8;
$countSql = 'SELECT COUNT(*) AS total FROM food';
...
$sql = 'SELECT food.f_id, food.nameFood, food.description, food.imageFood, food.price, category.name AS category_name
        FROM food
        LEFT JOIN category ON food.cart_id = category.c_id
        ORDER BY food.f_id DESC
...
<html lang="fr">
  <head>
    ...
    <title>Plats — Administration Restaurant Hôtel</title>
    ...
    <div class="admin-wrapper">
      ...
      <div class="content">
        <div class="topbar">
          ...
          <div class="brand-title">Plats</div>
          <div class="d-flex align-items-center">
            <a href="FoodList.php" class="btn btn-sm btn-primary"><i class="fa fa-plus"></i> Créer</a>
                  </div>
                </div>
                ...
                <table class="table table-hover">
                  ...
                  <tbody>
                    <?php while($row = mysqli_fetch_assoc($result)) { ?>
                      <tr>
                        <td>
                          <a class="btn btn-sm btn-outline-secondary" href="FoodList2.php?f_id=<?php echo urlencode($row['f_id']); ?>" title="Modifier"><i class="fa fa-pencil"></i></a>
                          <a class="btn btn-sm btn-outline-danger" href="deleteFood.php?f_id=<?php echo urlencode($row['f_id']); ?>" onclick="return confirm('Supprimer ce plat ?');" title="Supprimer"><i class="fa fa-trash"></i></a>
                            </td>
                        <td><img class="img-thumb" src="../../image/<?php echo htmlspecialchars($row['imageFood']); ?>" alt="<?php echo htmlspecialchars($row['nameFood']); ?>" /></td>
                        <td><?php echo htmlspecialchars($row['nameFood']); ?></td>
                ...
                <ul class="pagination pagination-sm mb-0">
                  <?php for ($i = 1; $i <= $number_of_pages; $i++) { ?>
                    <li class="page-item <?php echo ($i === $page) ? 'active' : ''; ?>">
                      <a class="page-link" href="food.php?page=<?php echo $i; ?>"><?php echo $i; ?></a>
                    </li>
                  <?php } ?>
                </ul>
              </nav>
