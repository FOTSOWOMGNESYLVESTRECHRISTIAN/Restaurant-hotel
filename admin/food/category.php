...
$sql = 'SELECT c_id, name, description, image, category_exixts FROM category ORDER BY c_id DESC LIMIT ' . $this_page_first_result . ', ' . $results_per_page;
...
<html lang="fr">
  <head>
    ...
    <title>Catégories — Administration Restaurant Hôtel</title>
    ...
    <div class="admin-wrapper">
      ...
      <div class="content">
        <div class="topbar">
          ...
          <div class="brand-title">Catégories</div>
          <div class="d-flex align-items-center">
            <a class="btn btn-sm btn-primary" href="CategoryList.php"><i class="fa fa-plus"></i> Créer</a>
                  </div>
                </div>
                ...
                <table class="table table-hover">
                  ...
                  <tbody>
                    <?php while($row = mysqli_fetch_assoc($result)) { ?>
                      <tr>
                        <td>
                          <a class="btn btn-sm btn-outline-secondary" href="CategoryList2.php?c_id=<?php echo urlencode($row['c_id']); ?>" title="Modifier"><i class="fa fa-pencil"></i></a>
                          <a class="btn btn-sm btn-outline-danger" href="deleteCategory.php?c_id=<?php echo urlencode($row['c_id']); ?>" onclick="return confirm('Supprimer cette catégorie ?');" title="Supprimer"><i class="fa fa-trash"></i></a>
                            </td>
                        <td><img class="img-thumb" src="../../image/<?php echo htmlspecialchars($row['image']); ?>" alt="<?php echo htmlspecialchars($row['name']); ?>" /></td>
                        <td><?php echo htmlspecialchars($row['name']); ?></td>
                        <td><?php echo nl2br(htmlspecialchars($row['description'])); ?></td>
                        <td><?php echo htmlspecialchars($row['category_exixts']); ?></td>
                          </tr>
                  <?php } ?>
                        </tbody>
                </table>
